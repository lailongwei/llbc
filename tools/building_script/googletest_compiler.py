...
class GoogleTestCompiler(object):
    """googletest 框架编译器"""
    def compile(self):
        """编译"""
        # 构建 cmake 构建目录
        ...
        if cfg.platform == PlatformType.Windows:
            self._compile_gtest_win32(gtest_fwk_path, cmake_build_dir)
        else:
            self._compile_gtest_non_win32(gtest_fwk_path, cmake_build_dir)

        Log.i('Compile googletest framework success')

    def _compile_gtest_win32(self, gtest_fwk_path, cmake_build_dir):
        """
        编译 googletest 测试框架 - Win32 平台
        """
        # 得到 cmake 构建工具路径(llb 为 windows 独立提供构建工具二进制)
        cmake_ver = '4.4.0'
...
/t:gtest;gmock /p:Platform={} /p:Configuration={} /p:ToolsVersion=Latest && popd'.format(
    ...
    def _compile_gtest_non_win32(self, gtest_fwk_path, cmake_build_dir):
        """
        编译 googletest 测试框架 - Non-Win32 平台
        """
