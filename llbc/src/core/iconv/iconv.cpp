...
win_iconv(iconv_t _cd, char **inbuf, size_t *inbytesleft, char **outbuf, size_t *outbytesleft)
{
    ...
    if (inbuf == NULL || *inbuf == NULL)
    {
        ...
        {
            tomode = cd->to.mode;
            outsize = cd->to.flush(&cd->to, (uchar *)*outbuf, static_cast<int>(*outbytesleft));
            if (outsize == -1)
            {
    ...
    while (*inbytesleft != 0)
    {
        ...
        insize = cd->from.mbtowc(&cd->from, (const uchar *)*inbuf, static_cast<int>(*inbytesleft), wbuf, &wsize);
        if (insize == -1)
        {
        ...
        outsize = cd->to.wctomb(&cd->to, wbuf, wsize, (uchar *)*outbuf, static_cast<int>(*outbytesleft));
        if (outsize == -1)
        {
            if ((cd->to.flags & FLAG_IGNORE) && errno != E2BIG)
            {
