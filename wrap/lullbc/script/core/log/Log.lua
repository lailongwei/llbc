...
-- trimFilePrefix: to control trim Log file info prefix character(&,=,@,...), default is false.
Log.trimFilePrefix = false
...
function Log.output(level, logger, tag, ...)
    ...
    if Log.logFileInfo then
        local di = debug_getinfo(3, 'Sl')
        file, line = di.source, di.currentline
        if Log.trimFilePrefix then
            file = string_sub(file, 2)
        end
    end
    ...
    if level >= 3 then -- For improve Log performance, explicit use Log Level value to perform if condition judge.
        local logMsg = Util_Table_Concat({...}, ' ')
        local logMsgAppendedTb = debug_traceback(logMsg, 3)
        LogMsg(level, logger, tag, file, line, logMsgAppendedTb)
    else
